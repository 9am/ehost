(function() {
  ...
  start = function(argv) {
    ...
    program.version("0.0.2").option("-l, --list", "list all host files").option("-c, --current", "print current host").option("-v, --view <name or *number>", "view host file").option("-s, --select <name or *number>", "set host active", util.list).parse(argv);
    if (program.list) {
      util.sl();
